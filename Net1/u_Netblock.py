...
class SAM(nn.Module):
    ...
    def forward(self, x, x_img):
        x1 = self.conv1(x)
        # 获取self.conv2(x)的维度信息
        conv2_output_shape = self.conv2(x).shape
        x_img_shape = x_img.shape
        # 对比维度，在维度不匹配的维度上进行插值调整（这里假设维度不匹配主要出现在第3个维度，你可根据实际情况调整维度判断逻辑）
        if conv2_output_shape[3]!= x_img_shape[3]:
            x_img = F.interpolate(x_img, size=(conv2_output_shape[2], conv2_output_shape[3]), mode='bilinear', align_corners=False)
