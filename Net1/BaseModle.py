...
class CorrelationLayer(nn.Module):  # 定义相关性层类，继承自 nn.Module
    def __init__(self, corr_size, feat_channel):  # 初始化方法，定义层的输入大小和特征通道数
        super(CorrelationLayer, self).__init__()  # 调用父类的初始化方法

        self.pool_layer = nn.AdaptiveAvgPool2d((corr_size, corr_size))  # 自适应平均池化层，将输入特征图调整为 corr_size * corr_size 的大小

        self.corr_reduce = nn.Sequential(  # 定义卷积层序列，用于降低相关性特征的维度
            nn.Conv2d(corr_size * corr_size, feat_channel, kernel_size=1),  # 1x1 卷积层，用于减少通道数
            nn.InstanceNorm2d(feat_channel),  # 实例归一化层
            nn.ReLU(),  # ReLU 激活函数
            nn.Conv2d(feat_channel, feat_channel, 3, padding=1),  # 3x3 卷积层，保持通道数不变并提取局部特征
        )
        self.Dnorm = nn.InstanceNorm2d(feat_channel)  # 实例归一化层，用于规范化特征图

        self.feat_adapt = nn.Sequential(  # 定义用于特征适配的卷积层序列
            nn.Conv2d(320, feat_channel, 1),  # 1x1 卷积层，用于通道数调整
            nn.InstanceNorm2d(feat_channel),  # 实例归一化层
            nn.ReLU(inplace=True)  # ReLU 激活函数，使用 inplace=True 节省内存
        )

    def forward(self, x):  # 前向传播方法，定义相关性计算的过程
        # 计算相关性图
        RGB_feat_downsize = F.normalize(self.pool_layer(x))  # 对特征图进行下采样并归一化
        RGB_feat_norm = F.normalize(x)  # 对输入特征图归一化
        RGB_corr, _ = corr_fun(RGB_feat_downsize, RGB_feat_norm)  # 调用 corr_fun 函数计算相关性图

        Red_corr = self.corr_reduce(RGB_corr)  # 使用 corr_reduce 处理相关性特征，减少通道数并提取局部特征

        # beta cond
        new_feat = torch.cat((x, Red_corr), dim=1)  # 将原始特征图与处理后的相关性特征在通道维度上拼接
        new_feat = self.feat_adapt(new_feat)  # 通过 feat_adapt 适配特征

        return new_feat  # 返回适配后的新特征
